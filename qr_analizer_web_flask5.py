...
def index():
    ...
    variants = [
        ...
        # Si quieres comparar contra la optimización por defecto:
    ...
    if request.method == 'POST':
        ...
        if not text:
            ...
            try:
                _ = segno.make(text, error=ecc, version=6, boost_error=False,
                ...
                for label, extra in variants:
                    for m in range(8):
                        try:
                            qr_v6 = segno.make(
                                text, error=ecc, version=6, boost_error=False,
                        ...
                        b64, metrics = render_colored_png_from_matrix(
                            matrix, qr_v6.version, ecc, getattr(qr_v6, 'mask', None)
                        )
                        qrs.append({
                            'version': qr_v6.version,   # 6
                            'size': metrics['size'],    # 41
                            'img_b64': b64,
                            'ecc': f"{ecc} · {label}",  # muestra variante
