#!/usr/bin/env python3
"""
QR Visual Analyzer (full) — Solo versión 6 (41x41) y TODAS las máscaras 0..7

- Flask app que genera únicamente la versión 6 (41×41 módulos) para un texto y ECC dados.
...
<head>
  <meta charset="utf-8">
  <title>QR Visual Analyzer — v6 (41×41) + Masks 0..7</title>
  <style>
...
<body>
  <h1>QR Visual Analyzer — Solo versión 6 (41×41), todas las máscaras</h1>
  ...
  {% if qrs %}
    <p>Se generan <strong>únicamente</strong> símbolos de <strong>versión 6 (41×41 módulos)</strong>, probando <strong>todas las máscaras (0..7)</strong> con el ECC seleccionado.</p>
    <div class="grid">
      {% for info in qrs %}
      <div class="card">
    ...
    <div class="legend">
      ...
      <div><span class="sw" style="background:rgb(180,0,0)"></span> Version bits (v>=7; en v6 no aplica)</div>
...
def build_function_mask(size, version):
    ...
    # Version information for v>=7 (en v6 no aplica, pero lo dejamos general)
    if version >= 7:
        for r in range(0,6):
...
def index():
    ...
    if request.method == 'POST':
        ...
        if not text:
            ...
            # Validar que el texto cabe en VERSIÓN 6 con el ECC seleccionado.
            try:
                # probar con una máscara cualquiera (0) solo para validar cabida en v6
                _ = segno.make(text, error=ecc, version=6, boost_error=False, micro=False, mask=0)
            except Exception:
                error = "El texto no cabe en versión 6 (41×41) con el ECC seleccionado."
            else:
                # Generar las 8 máscaras (0..7) exclusivamente para versión 6
                for m in range(8):
                    try:
                        qr_v6 = segno.make(text, error=ecc, version=6, boost_error=False, micro=False, mask=m)
                    except Exception:
                        # No debería fallar tras la validación, pero por si acaso
                        continue
                    matrix = list(qr_v6.matrix)
                    b64, metrics = render_colored_png_from_matrix(matrix, qr_v6.version, ecc, getattr(qr_v6, 'mask', None))
                    qrs.append({
                        'version': qr_v6.version,         # siempre 6
                        'size': metrics['size'],           # 41
                        'img_b64': b64,
                        'ecc': ecc,
                        'mask': getattr(qr_v6, 'mask', None),
                ...
                if not qrs:
                    error = "No se pudo generar ninguna máscara para v6 (caso inesperado)."
